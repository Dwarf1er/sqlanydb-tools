...
} from "@sqlanydb-tools/sqlanydb-config";

export const startDatabase = async (
    databaseName: string,
    databaseConfigurationManager: DatabaseConfigurationManager
): Promise<void> => {
    const databaseConfiguration = await findDatabaseConfiguration(
        databaseName,
        databaseConfigurationManager
    );
    ...
    if (databaseConfiguration.serverPort) {
        commandParts.push(
            `-x TCPIP(port=${databaseConfiguration.serverPort};DOBROAD=NO)`
        );
    }

    if (databaseConfiguration.httpPort) {
        commandParts.push(
            `-xs http(port=${databaseConfiguration.httpPort};dbn=${databaseConfiguration.name})`
        );
    }
...
export const resetDatabase = async (
    ...
    if (!fs.existsSync(databaseConfiguration.archivePath)) {
        throw new Error(
            `Archive not found: ${databaseConfiguration.archivePath}`
        );
    }

    await stopDatabase(databaseName, databaseConfigurationManager).catch(
        (err) => {
            console.warn(
                `Database is not running or could not stop database ${databaseName}: ${err.message}`
            );
        }
    );
    ...
    try {
        const logFiles = (
            await fs.promises.readdir(databaseConfiguration.path)
        ).filter((logFile) => logFile.endsWith(".log"));
    ...
    return new Promise((resolve, reject) => {
        fs.createReadStream(archivePath)
            ...
            .on("close", async () => {
                const extractedFiles = await fs.promises.readdir(
                    databaseConfiguration.path
                );
                const dbFiles = extractedFiles.filter((file) =>
                    file.endsWith(".db")
                );
                ...
                if (dbFiles.length === 1) {
                    ...
                    resolve(
                        `Database ${databaseName} has been reset from archive: ${archivePath}`
                    );
                } else {
                    reject(
                        new Error(
                            `Expected one .db file after extraction, found: ${dbFiles.length}`
                        )
                    );
                }
            })
            .on("error", (error) => {
                reject(
                    new Error(`Failed to extract archive: ${error.message}`)
                );
            });
    });
};
...
const exponentialBackoff = (attempt: number) =>
    Math.min(10000, Math.pow(2, attempt) * 100);
