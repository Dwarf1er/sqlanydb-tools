...
export async function resetDatabaseCommand(
	...
	return vscode.commands.registerCommand("sqlanydb-tools-vscode.resetDatabase", async () => {
		...
		if (databases.length > 0) {
			...
			await vscode.window.withProgress(
				...
				async () => {
					...
					await checkAndUpdateDatabaseStatus(treeDataProvider, databaseConfigurationManager);
				}
			);
		}
	});
